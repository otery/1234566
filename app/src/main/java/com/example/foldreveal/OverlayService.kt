...
class OverlayService : Service() {
    ...
    private lateinit var foldingTracker: FoldingFeatureTracker
    ...
    override fun onCreate() {
        ...
        foldingTracker = FoldingFeatureTracker(this)
        foldingTracker.start(serviceScope)
    ...
    private fun beginTransition(angle: Float) {
        ...
        overlayView?.apply {
            // 기기가 보고하는 실제 접힘선을 렌더러에 반영합니다.
            if (foldingTracker.hasFoldingFeature) {
                hingePositionRatio = foldingTracker.hingePositionRatio
                hingeIsVertical = foldingTracker.isVertical
            }
        ...
        coverPresentation?.apply {
            if (foldingTracker.hasFoldingFeature) {
                setHingeGeometry(foldingTracker.hingePositionRatio, foldingTracker.isVertical)
            }
            setAngleDegrees(angle)
            startRendering(initial, angle)
        }
